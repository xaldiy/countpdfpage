...
========================================
- C: artinya creation atau tanggal pembuatan
- M: artinya modification atau tanggal perubahan (edit file pdf atau pindah lokasi atau ubah nama file)
""", "yellow", style="bold"))
...
def input_folder():
    ...
    print(color_text("║ Contoh: ", "cyan") + color_text(r"D:\Arsip\Scanned Document", "cyan", style="bold"))
    print(color_text("╚═══════════════════════════════════════════════╝", "cyan"))
...
def input_date_filter():
    ...
    print(color_text("1. Hanya yang dibuat HARI INI", "green"))
    print(color_text("2. Berdasarkan tanggal tertentu (contoh: 03-11-2025 atau 3 November 2025)", "yellow"))
    print(color_text("3. Scan SEMUA file tanpa filter tanggal (Modified all directory)", "magenta", style="bold"))
    print(color_text("Tekan ENTER untuk default ke Opsi 3 (Scan SEMUA)", "white"))
    choice = input(color_text("    ➡️  ", "green", style="bold")).strip()

    if choice == "1":
        return datetime.now().date(), datetime.now().date(), False
    elif choice == "2":
        ...
        try:
            ...
            return None, datetime.now().date(), True
        ...
        try:
            ...
            print(color_text("⚠️ Pilihan tidak dikenali, akan discan semua file (Opsi 3).", "yellow"))
            return None, datetime.now().date(), True
...
def process_folder(folder, date_filter=None, modified_date_target=None, scan_all_mode=False):
    try:
    ...
    for fn in filenames:
        ...
        try:
            c_ts = os.path.getctime(path)
            m_ts = os.path.getmtime(path)
            ctime = datetime.fromtimestamp(c_ts).date()
            mtime = datetime.fromtimestamp(m_ts).date()
        except Exception:
            continue

        is_modified = (mtime != ctime) and (mtime == modified_date_target)
    ...
    if date_filter:
        ...
        print(color_text(f"\n🗂️  Folder: {folder_name} (Filter: SEMUA file)", "red", style="bold"))
    print(color_text("──────────────────────────────────────────────", "blue"))

    if not filtered_files:
        print(color_text("⚠️ Tidak ada file PDF yang ditemukan sesuai filter tanggal.", "yellow"))
        return {
            "folder": folder_name,
            "total_files": 0,
            "total_pages": 0,
            ...
            "duplicates": [], "dup_count": 0,
            "anomalies": [], "anom_count": 0,
            "notes": color_text("Tidak ada data sesuai filter.", "yellow")
        }
    ...
    for detail in file_details_list:
        ...
        page_text = (color_text(f"{pages} lembar", "green") if pages is not None else color_text("❌ gagal baca", "red"))
        
        info = f"[C:{ctime.strftime('%d-%m')} M:{mtime.strftime('%d-%m')}]"
        
        if scan_all_mode:
             line_color = "red"
        elif ctime == modified_date_target:
             line_color = "white"
        else:
             line_color = "yellow"

        formatted_files.append(color_text(f"- {fn:<25} {info:<18}{page_text}", line_color))
        
    print_table(formatted_files)

    candidates = [extract_candidate_number(detail["fn"]) for detail in file_details_list if extract_candidate_number(detail["fn"])]
...
def pretty_print_results(results, date_filter=None, modified_date_target=None):
    print(color_text("\n\n🔥 HASIL AKHIR 🔥\n", "magenta", style="bold"))
    
    total_modified = sum(r.get('modified_match', 0) for r in results)
    
    if date_filter is None:
        print(color_text("MODE: SCAN SEMUA FILE TANPA FILTER TANGGAL", "red", style="bold"))
        if modified_date_target:
            print(color_text(
                f"PERHATIAN: Data di bawah adalah semua file PDF di direktori. "
                f"Total Modified dihitung berdasarkan file yang dimodifikasi di tanggal {modified_date_target.strftime('%Y-%m-%d')}.",
                "red"
            ))
        else:
            print(color_text("PERHATIAN: Mode scan semua file tanpa filter tanggal aktif.", "red"))
    else:
        print(color_text(f"MODE: FILTER TANGGAL (Created Date) = {date_filter.strftime('%Y-%m-%d')}", "cyan"))
        print(color_text(f"FILE YANG DIMODIFIKASI DI TANGGAL {date_filter.strftime('%Y-%m-%d')} "
                         f"TIDAK DIHITUNG KECUALI CREATED DATE SAMA DENGAN {date_filter.strftime('%Y-%m-%d')}", "cyan"))
    
    total_files = total_pages = total_missing = total_dup = total_anom = 0

    for r in results:
        print(color_text("Nama folder ", "cyan") + color_text(r["folder"], "yellow", style="bold"))
        print(color_text("--------------------------------------------------", "blue"))
    ...
    if date_filter is None:
        print(color_text(f"Total semua file PDF yang discan: {total_files} (SEMUA FILE TERMASUK YANG DIMODIFIKASI)", "red"))
        if modified_date_target:
            print(color_text(f"Total modified file PDF di tanggal {modified_date_target.strftime('%Y-%m-%d')}: {total_modified}", "yellow"))
    else:
        print(color_text(f"Total semua file PDF yang dihitung: {total_files} (Filter Created at {date_filter.strftime('%Y-%m-%d')})", "white"))
    ...
    print(color_text("--------------------------------------------------", "blue"))
    print(color_text("- C: artinya creation atau tanggal pembuatan", "yellow", style="bold"))
    print(color_text("- M: artinya modification atau tanggal perubahan (edit file pdf atau pindah lokasi atau ubah nama file)", "yellow", style="bold"))
    print(color_text("--------------------------------------------------", "blue"))

    if date_filter and modified_date_target:
        print(color_text(
            f"💡 Kalau file dimodifikasi tanggal {modified_date_target.strftime('%Y-%m-%d')}, "
            f"belum tentu dibuat di tanggal {date_filter.strftime('%Y-%m-%d')}. "
            f"Ditemukan total {total_modified} file yang dimodifikasi tanggal {modified_date_target.strftime('%Y-%m-%d')}, "
            f"tapi kalau tidak terhitung dalam total file yang dihitung, berarti file tersebut tidak dibuat di tanggal filter.",
            "cyan"
        ))
    elif modified_date_target:
        print(color_text(
            f"💡 Mode scan semua file aktif — ditemukan total {total_modified} file yang dimodifikasi "
            f"pada tanggal {modified_date_target.strftime('%Y-%m-%d')}.",
            "cyan"
        ))
    else:
        print(color_text("💡 Tidak ada informasi tanggal modifikasi yang tersedia.", "cyan"))
...
if __name__ == "__main__":
    ...
    date_filter, modified_date_target, scan_all_mode = input_date_filter()
    ...
    for f in pdf_folders:
        res = process_folder(f, date_filter, modified_date_target, scan_all_mode)
        results.append(res)

    pretty_print_results(results, date_filter, modified_date_target)
